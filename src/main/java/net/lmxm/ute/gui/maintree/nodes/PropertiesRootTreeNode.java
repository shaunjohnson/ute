...
public final class PropertiesRootTreeNode extends AbstractRootTreeNode {
	...
	protected String convertToString(final Configuration configuration) {
		final String format = ResourcesUtils.getResourceText(TreeNodeResourceType.PROPERTIES);

		return String.format(format, configuration.getProperties().size());
	}
    ...
    public final BeanType getType() {
        return BeanType.PropertiesRootTreeNode;
    }
}
